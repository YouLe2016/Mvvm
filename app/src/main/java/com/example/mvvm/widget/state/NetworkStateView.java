...
import com.example.mvvm.utils.UiUtils;


/**
 * Created by haoran on 2017/4/15.
 * 加载状态的自定义View
 */

public class NetworkStateView extends LinearLayout implements NetworkStateAction {
    ...
    private OnRetryClickListener mRefreshListener;
    ...
    public NetworkStateView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NetworkStateView, defStyleAttr, R.style.NetworkStateViewStyle);
    ...
    public void showError() {
        mCurrentNetworkState = NetworkState.STATE_NETWORK_ERROR;
        if (null == mErrorView) {
    ...
    public void showNoNetworkRetry() {
        mCurrentNetworkState = NetworkState.STATE_NO_NETWORK_RETRY;
    ...
    public void showEmpty() {
        mCurrentNetworkState = NetworkState.STATE_EMPTY;
        if (null == mEmptyView) {
    ...
    private void showViewByState(@NetworkState int state) {

        //如果当前状态为加载成功，隐藏此View，反之显示
        this.setVisibility(state == NetworkState.STATE_SUCCESS ? View.GONE : View.VISIBLE);

        if (null != mLoadingView) {
            mLoadingView.setVisibility(state == NetworkState.STATE_LOADING ? View.VISIBLE : View.GONE);
        }

        if (null != mErrorView) {
            mErrorView.setVisibility(state == NetworkState.STATE_NETWORK_ERROR ? View.VISIBLE : View.GONE);
        }

        if (null != mNoNetworkView) {
            mNoNetworkView.setVisibility(state == NetworkState.STATE_NO_NETWORK_RETRY ? View.VISIBLE : View.GONE);
        }

        if (null != mEmptyView) {
            mEmptyView.setVisibility(state == NetworkState.STATE_EMPTY ? View.VISIBLE : View.GONE);
        }
    }
